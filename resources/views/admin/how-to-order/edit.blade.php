...
@section('content')
    <form action="{{ route('admin.how-to-orders.update', $howToOrder) }}" method="POST" id="howToOrderForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-5 space-y-5">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all leading-relaxed min-h-[76px] max-h-36 resize-y">{{ old('description', $howToOrder->description) }}</textarea>
                            @error('description')
                    ...
                    <div class="pt-4 border-t border-gray-100 space-y-2 text-xs">
                        <div class="flex justify-between items-start text-slate-500">
                            ...
                            <div class="text-right">
                                <span class="text-slate-700 font-medium">{{ $howToOrder->updated_at->translatedFormat('d M Y, H:i') }} WIB</span>
                                <span class="text-[11px] text-slate-400 block">({{ $howToOrder->updated_at->diffForHumans() }})</span>
                            </div>
                        </div>
