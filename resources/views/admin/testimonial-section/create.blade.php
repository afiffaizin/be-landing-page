...
@section('title', 'Buat Section Testimonial')
...
@section('page_title', 'Buat Section Testimonial')
...
@endsection
@section('page_headline', 'Create Testimonial Section')
@section('page_subtitle', 'Kelola judul utama, statistik dampak, dan daftar ulasan pelanggan.')
...
@section('content')
    <form action="{{ route('admin.testimonial-sections.store') }}" method="POST" id="testimonialForm">
        @csrf
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-xs overflow-hidden p-6">
                    ...
                    <div id="stats-container" class="space-y-4">
                        @php $oldStats = old('statistics', []); @endphp
                        @if (count($oldStats) > 0)
                            @foreach ($oldStats as $idx => $stat)
                                <div class="stat-item p-4 rounded-xl border border-slate-200 bg-slate-50/40 relative">
                                    <div class="flex items-center justify-between mb-4 border-b border-slate-200/50 pb-2 cursor-pointer" onclick="toggleAccordion(this)">
                                        <div class="font-bold text-xs text-slate-700 stat-header-title">Item Statistik</div>
                                    ...
                                    <div class="space-y-4 accordion-body bg-white p-3 rounded-lg border border-slate-100">
                                        <div>
                                            ...
                                            <input type="text" name="statistics[{{ $idx }}][value]" value="{{ $stat['value'] ?? '' }}" required class="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600" placeholder="Cth: 350+ kg">
                                        </div>
                                        <div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-xs overflow-hidden p-6">
                    ...
                    <div id="testimonials-container" class="space-y-4">
                        @php $oldTestimonials = old('testimonials', []); @endphp
                        @if (count($oldTestimonials) > 0)
                            @foreach ($oldTestimonials as $idx => $item)
                                <div class="testimonial-item p-4 rounded-xl border border-slate-200 bg-slate-50/40 relative">
                                    <div class="flex items-center justify-between mb-4 border-b border-slate-200/50 pb-2 cursor-pointer" onclick="toggleAccordion(this)">
                                        <div class="font-bold text-xs text-slate-700 testimonial-header-title">Item Testimoni</div>
                                        <div class="flex items-center gap-2">
                                    ...
                                    <div class="space-y-4 accordion-body bg-white p-3 rounded-lg border border-slate-100">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input type="text" name="testimonials[{{ $idx }}][name]" value="{{ $item['name'] ?? '' }}" required class="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600" placeholder="Cth: Ibu Endang Rahayu">
                                            </div>
                                            <div>
...
@push('scripts')
    <script>
        ...
        // Trigger click if empty on load
        if(statIndex === 0) addStatBtn.click();
        if(testimonialIndex === 0) addTestimonialBtn.click();
