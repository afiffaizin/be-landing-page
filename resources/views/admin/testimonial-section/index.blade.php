...
@section('breadcrumbs')
    ...
    <span>/</span>
    <span class="text-slate-800">Testimonial</span>
@endsection

@section('page_title', 'Kelola Section Testimonial')

@section('header_actions')
    <a href="{{ route('admin.testimonial-sections.create') }}"
        ...
        <span>Buat Section Testimonial</span>
    </a>
@endsection

@section('page_headline', 'Daftar Section Testimonial')
@section('page_subtitle', 'Kelola informasi testimonial dan statistik dampak di landing page.')
@section('status_badge')
    <span
        ...
        {{ \App\Models\TestimonialSection::where('is_active', true)->count() }} Section Aktif Utama
    </span>
@endsection
...
@section('content')
    <div class="space-y-5">
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs">
            <form action="{{ route('admin.testimonial-sections.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto shrink-0">
                    ...
                    <div class="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        @if(request('search') || request('status'))
                            <a href="{{ route('admin.testimonial-sections.index') }}" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-semibold rounded-xl transition-all" title="Reset Filter">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl border border-gray-200/80 shadow-xs overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="py-3.5 px-5">Statistik & Testimoni</th>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="py-3.5 px-5 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <span
                                            ...
                                            {{ $section->statistics->count() }} Statistik
                                        </span>
                                        <span
                                            class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-blue-50 text-blue-700 border border-blue-200/80">
                                            <svg class="w-3.5 h-3.5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                                            </svg>
                                            {{ $section->testimonials->count() }} Testimoni
                                        </span>
                                    </div>
                                </td>
                                <td class="py-3.5 px-5 whitespace-nowrap text-center">
                                    <form action="{{ route('admin.testimonial-sections.toggle-status', $section) }}"
                                ...
                                <td class="py-3.5 px-5 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <a href="{{ route('admin.testimonial-sections.edit', $section) }}"
                                        ...
                                        <form id="delete-form-{{ $section->id }}"
                                            action="{{ route('admin.testimonial-sections.destroy', $section) }}" method="POST"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-16 text-center">
                                    ...
                                    <h4 class="text-sm font-bold text-slate-800">Belum Ada Section Testimonial</h4>
                                    <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                                        Tambahkan section Testimonial pertama Anda.
                                    </p>
                                    <a href="{{ route('admin.testimonial-sections.create') }}"
            ...
            @if ($testimonialSections->hasPages())
